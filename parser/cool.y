...
%{
  ...
  for the location of tokens */
   ...
   // p(9); 
	...
	formal_list1:
    formal	{ $$ = single_Formals($1); //p(11); 
    }
    | formal_list1 ',' formal	{ $$ = append_Formals($1, single_Formals($3));// p(12);
     }
    ;
    
    formal:
    OBJECTID ':' TYPEID	{ $$ = formal($1, $3); //p(13); 
    }
    ;
    ...
    | expr_list1 ',' expr	{ $$ = append_Expressions($1, single_Expressions($3)); }
    ;
    
    expr_stmts:
    expr ';' 				{ $$ = single_Expressions($1); }
	| expr_stmts expr ';' 	{ $$ = append_Expressions($1, single_Expressions($2)); }
	| error ';'	
	;
	
	case_list:
	single_case				{ $$ = single_Cases($1); }
	| case_list single_case { $$ = append_Cases($1, single_Cases($2)); }
	;
	...
	let_assignment:
	OBJECTID ':' TYPEID IN expr	%prec LETA{ $$ = let($1, $3, no_expr(), $5); }
	| OBJECTID ':' TYPEID ASSIGN expr IN expr %prec LETA { $$ = let($1, $3, $5, $7); }
	| OBJECTID ':' TYPEID ',' let_assignment %prec LETA	{ $$ = let($1, $3, no_expr(), $5); }
	| OBJECTID ':' TYPEID ASSIGN expr ',' let_assignment %prec LETA
	{ $$ = let($1, $3, $5, $7); }
	| error IN expr	%prec LETA
	| OBJECTID ':' TYPEID IN error
	;
	
	let_stmt:
	LET let_assignment	{ $$ = $2; }
	;
