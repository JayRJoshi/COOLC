...
public:
   ...
   virtual Symbol traverse(TypeEnv *) = 0;
...
public:
   ...
   bool is_type_method() { return is_method; }
   virtual Symbol traverse(TypeEnv *) = 0;
...
typedef class class__class *Classp;
...
typedef class method_class *Methodp;
...
typedef class attr_class *Attrp;
...
typedef class formal_class *Formalp;
...
typedef class branch_class *Branchp;
...
public:
	...
	TypeEnv(ClassTable *_ct, SymbolTable<Symbol, Symbol> *ot, SymbolTable<Symbol, Symbol> *mt) {
		ct = _ct;
		objecttable = ot;
		methodtable = mt;
	}
	...
	void addid(Symbol s, Symbol c) {
		Symbol *tmp = new Symbol;
		*tmp = c;
		objecttable->addid(s, tmp);
	}
	
	bool hasid(Symbol s) {
		return objecttable->lookup(s)!=NULL;
	}
	
	bool probe(Symbol s) {
		return objecttable->probe(s) != NULL;
	}
	
	bool hasid(Feature f) {
		return objecttable->lookup(((Methodp) f)->get_name()) != NULL;
	}
	
	bool probe(Feature f) {
		return objecttable->probe(((Methodp) f)->get_name()) != NULL;
	}
	
	Symbol getid(Symbol c) {
		return *objecttable->lookup(c);
	}
	
	void set_curr_class(Symbol c) {
		curr_class = c;
	}
	...
	Methodp get_method(Symbol cname, Symbol mname, Symbol cl);
	bool valid_signature(Expressions f1, Methodp mp);
	ostream& semant_error();
	Symbol lcu(Symbol t1, Symbol t2, Symbol cl);
};
#endif
